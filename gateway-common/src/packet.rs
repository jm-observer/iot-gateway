use crate::pub_use::*;
use crate::{
    get_index_group_str, uuid, CloneJson, ExtObject, Global, IntoResult, MQTT_ACK_CODE,
    MQTT_ACK_CODE_ERR, MQTT_ACK_CODE_OK, MQTT_ACK_MSG, MQTT_SELF_TYPE, MQTT_SERVER,
};
/// mqtt报文的相关操作
///
///
...
impl MqttPacket {
    ...
    #[allow(unused_variables)]
    // todo 优化掉
    pub async fn ack_fail_from_failtype(&self, fail_type: &str) {
        let mut msg = Json::new();
        msg.ext_add_str_object(MQTT_ACK_CODE, "1000")
            .ext_add_str_object(MQTT_ACK_MSG, fail_type);
        match self.general_ack_packet(msg) {
            Ok(ack) => self.global.send_to_mqtt(ack).await,
            Err(e) => {
                error!("{:?}", e);
