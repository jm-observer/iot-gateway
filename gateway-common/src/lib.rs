...
pub fn deal_file(tmp_file: &str, target_file: &str) -> Result<()> {
    Command::new("mv").arg(tmp_file).arg(target_file).status()?;
    Command::new("chmod").arg("777").arg(target_file).status()?;
    Command::new("sudo")
        .arg("systemctl")
        .arg("daemon-reload")
        .status()?;
...
pub trait IntoResult<T> {
    fn into_result_by_msg<A>(self, msg: A) -> Result<T>
    where
        A: std::fmt::Debug;

    fn into_result(self) -> Result<T>;
}
impl<T, B> IntoResult<T> for std::result::Result<T, B>
where
    B: std::fmt::Debug,
{
    fn into_result_by_msg<A>(self, msg: A) -> Result<T>
...
impl<T> IntoResult<T> for Option<T> {
    ...
    fn into_result(self) -> Result<T> {
        self.into_result_by_msg("option的值为空")
    }
}
