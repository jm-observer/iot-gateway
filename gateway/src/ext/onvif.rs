...
fn get_ipc_uuid_addr(
    ...
    let cap = reg
        .captures(addr.as_str())
        .into_result_by_msg(format!("XAddrs[{}]地址不规范", addr))?;
...
pub(crate) async fn check_device_password_by_cloud(
    ...
    let url = tmp.into_result_by_msg("该ipc离线中...")?;
    let lock = global.ipc_ips.read().await;
    let tmp = lock.get(uuid);
    let ip = tmp.into_result_by_msg("该ipc离线中...")?;
...
fn _get_profile<'a>(doc: &'a Document) -> Result<Profile> {
    ...
    let node = nodes.into_result_by_msg("找不到Profiles节点")?;
    _init_profile(node)
}
