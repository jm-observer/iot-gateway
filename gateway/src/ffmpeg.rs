...
use gateway_common::VideoCommand::{CloudEndVideo, ReqImages};
        ...
        match command {
            VideoCommand::ReqVideo(packet) => {
                ...
                let rtmp = match _get_rtmp_addr(global.clone()).await {
                    Ok(tmp) => tmp,
                    Err(e) => {
                        packet.ack_fail_from_failtype(e.to_string().as_str()).await;
                        continue;
                    }
                };
...
async fn req_video(packet: Arc<MqttPacket>, rtmp: &str) -> Result<Child> {
    ...
    let ip = ip_tmp.get(&uuid_tmp).into_result_by_msg("该ipc离线中！")?;
