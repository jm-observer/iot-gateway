...
impl Toml {
    ...
    pub async fn update(&self) -> Result<()> {
        let res = async_std::fs::write(&self.path, self.config.to_string()).await;
        fail_from_result(res)
    }
    ...
    pub async fn add_ipc_config(
        ...
        let cap = fail_from_option(
            self.reg.captures(stream_url.as_str()),
            &format!("stream_url[{}]不规范！", stream_url),
        )?;
    ...
    pub fn get_ipcs(&self) -> Result<Vec<String>> {
        ...
        let tmp = fail_from_option(
            self.config["onvif"]["ipc"].as_table(),
            "配置节点[onvif.ipc]异常",
        )?;
    ...
    pub fn get_ipc_data(&self, uuid: &str) -> Result<(String, String, String, String, String)> {
        ...
        if ipc.is_none() {
            return fail_from_str("该ipc未鉴权");
        }
        let ipc = ipc.unwrap();
        Ok((
            uuid.to_string(),
            self._get_item_string(fail_from_option(ipc.get("name"), "该ipc配置信息缺失")?)?,
            self._get_item_string(fail_from_option(ipc.get("password"), "该ipc配置信息缺失")?)?,
            self._get_item_string(fail_from_option(
                ipc.get("stream_url"),
                "该ipc配置信息缺失",
            )?)?,
            self._get_item_string(fail_from_option(
    ...
    pub fn remove_ipc_config(&mut self, uuid: &str) -> Result<bool> {
        if self.config["onvif"]["ipc"][uuid].is_none() {
            return fail_from_str("该uuid不存在，无法删除");
        }
    ...
    pub async fn update_ipc_auth(
        ...
        if self.config["onvif"]["ipc"][uuid].is_none() {
            return fail_from_str("当前ipc不存在，需要重新扫描");
        }
        self.config["onvif"]["ipc"][uuid]["name"] = value(user);
        self.config["onvif"]["ipc"][uuid]["password"] = value(password);
    ...
    pub fn get_ipc_rtsp(&self, uuid: &str) -> Result<String> {
        ...
        fail(format!("ipc[{}]不存在，或者未进行密码认证！", uuid))
    }
    ...
    pub fn _get_value_string(&self, item: &Value) -> Result<String> {
        match item.as_str() {
            ...
            None => fail(format!("item[{:?}]的值获取失败", item)),
        }
    }
    pub fn _get_item_string(&self, item: &Item) -> Result<String> {
        if item.is_none() {
            return fail_from_str("配置节点不存在，无法获取配置信息");
        }
        match item.as_str() {
            Some(val) => Ok(val.to_string()),
            None => fail(format!("item[{:?}]的值获取失败", item)),
    ...
    pub fn _get_item_array_string(&self, item: &Item) -> Result<Vec<String>> {
        ...
        if let Some(array) = item.as_array() {
            ...
            fail(format!("item[{:?}]的值转成数组失败", item))
        }
    }
    ...
    pub fn _get_item_f64(&self, item: &Item) -> Result<f64> {
        ...
        fail_from_option(item.as_float(), "节点值不为float")
    }
    pub fn _get_item_int(&self, item: &Item) -> Result<i64> {
        if item.is_none() {
            return fail_from_str("配置节点不存在，无法获取配置信息");
        }
        fail_from_option(item.as_integer(), "节点值不为int")
    }
