...
pub async fn env_collect_task(global: Arc<Global>) {
    ...
    loop {
        ...
        match env_serial_task_detail(global.clone()).await {
            ...
            Err(FailTypeEnum::SerialInterInactive(err)) => {
                //未启用，则无其他操作
                // warn!("{}", err);
                if inavtive > inavtive_time {
                    report_info_msg(global.clone(), err.as_str()).await;
                    inavtive = 0u64;
                } else {
                    inavtive = inavtive + frequency;
                }
            }
            Err(FailTypeEnum::Fail(err)) => {
                // warn!("{}", err);
                if fail > fail_time {
                    report_error_msg(global.clone(), err.as_str()).await;
                    fail = 0u64;
                } else {
                    fail = fail + frequency;
                }
...
async fn _screen_control_detail(global: Arc<Global>) -> Result<()> {
    ...
    if screen_on_time > screen_off_time {
        return fail(format!(
            ...
            screen_on_time, screen_off_time
        ));
    }
